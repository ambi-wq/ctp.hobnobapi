...
from .serializers import ProfileSerializer, InterestSerializer, UserInterestSerializer, UserImageSerializer, \
    UserPreferenceSerializer, InstagramSerializer, PromptQuestionsSerializer, ReplaceQuestionSerializer, \
    DowntoChillSerializer, UserImagesCommentSerializer
from .serializers import OtherUserProfileSerializer, SpotifySerializer, ProfileDataSerializer, DisplayPictureSerializer, \
    UserPromptQASerializer, UserCreatePromptQASerializer, ContactSerializer, ProfileDetailsSerializer, \
    AllUserImageCommentsSerializer, UserProfileImageDetailsSerializer
...
from .models import ProfileDetails, Interest, UserImages, UserPreference, InstagramConnect, SpotifyConnect, \
    DisplayPicture, UserPromptQA, PromptQuestions, UserContactList, UsersContacts, UserImagesComment
...
class UserPofileImg(views.APIView):
    ...
    def get(self, request):
        ...
        result_page = paginator.paginate_queryset(user_image, request)
        user_img_serializer = UserImageSerializer(result_page, many=True)
        return paginator.get_paginated_response(user_img_serializer.data)
    ...
    if ProfileDetails.objects.filter(uuid=user).exists() and ProfileDetails.objects.get(uuid=user).active == True:
        ...
        if not is_account_connected:
            try:
                ...
                return Response(data={"message": "Access Token or Refresh Token or Both are missing"},
...
class DowntoChillList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        print("following_list", following_list)
        return ProfileDetails.objects.filter(~Q(uuid=current_user), uuid__in=following_list,
                                             is_available=True)  # .exclude(uuid__in=following_list))
...
class ProfileUsername(views.APIView):
    ...
    def get(self, request, username):
        ...
        if is_username_exist:
            ...
            # return Response(standard_response, status=status.HTTP_200_OK)
            return HttpResponse(json.dumps(standard_response), content_type='application/json')
        else:
        ...
        try:
            ...
            if pref_exist == 0:
                ...
                if request.version == 'v1':
                    ...
                    return Response({"message": "User Preference already exist. Please use update method for update."},
    ...
    def get(self, request):
        ...
        if instagram_user.count() > 0:
            ...
            url = "{root_url}/{insta_id}/media?fields=id,username,media_url&access_token={access_token}".format(
                root_url=settings.INSTAGRAM_ROOT_URL, insta_id=user_data.instagram_id,
        ...
        if 'code' in request.data:
            ...
            if response.status_code == 400:
                ...
                url = "{root_url}/access_token?client_secret={app_secret}&access_token={short_token}&grant_type=ig_exchange_token".format(
                    root_url=settings.INSTAGRAM_ROOT_URL, app_secret=settings.INSTA_APP_SECRET,
                    short_token=short_live_token)
...
class SpotifyView(views.APIView):
    ...
    def spotify_helper(self, url, grant_type, token_or_code):
        ...
        if grant_type == 'authorization_code':
            print("entered if")
            payload = 'grant_type={0}&code={1}&redirect_uri={2}&client_id={3}&client_secret={4}'.format(
                grant_type, token_or_code, 'https://api.hobnobco.com/', settings.SPOTIFY_CLIENT_ID,
    ...
    def post(self, request):
        ...
        try:
            ...
            if len(contactListRecord) > 0:
                ...
                get_record.delete()
                create_contacts_list = UserContactList.objects.create(user=current_user,
                                                                      contacts_list=request.data['contacts_list'])
...
def delete_user(request):
    ...
    try:
        ...
        # comment
        # current_user = "u8BNKvT5wEZCNIeACUVpCFvpPI13"

        firebase_admin.delete_app(firebase_admin.get_app())
        cred = credentials.Certificate("credentials/serviceAccountKey.json")
        firebase_admin.initialize_app(cred, {'databaseURL': 'https://hobnob-aum.firebaseio.com/'})

        ref = db.reference("/Users/{0}/".format(current_user))
        data = ref.get()

        if data:
            if "conversations" in data.keys():
                conversation_ref = ref.child("conversations")
                for key, value in conversation_ref.get().items():
                    # Get the channel id
                    channel_id = conversation_ref.child("{0}/channelId".format(key)).get()
                    # Delete the chat of that user from Channels table
                    # Uncomment while deploying
                    channel_ref = db.reference("Channels/{0}".format(channel_id)).set({})
            ...
            # Success Msg
            success_msg = {"message": "User deleted successfully", "status": True}
        else:
            success_msg = {"message": "User not found.", "status": False}

        if request.version == 'v1':
            response = success_response(success_msg)
            return Response(response, status=status.HTTP_200_OK)
        else:
            return Response(success_msg, status=status.HTTP_200_OK)
...
class UserImageLiked(views.APIView):
    ...
    def post(self, request, id):
        try:
            ...
            if ProfileDetails.objects.filter(uuid=user).exists() and ProfileDetails.objects.get(
                ...
                if UserImages.objects.filter(id=id, liked_user=user).exists():
                    # success message
                    success_msg = {"message": "User already liked your photo", "status": False}

                else:
                    images = UserImages.objects.get(id=id)
                    images.liked_user.add(user)
                    images.save()

                    # success message
                    success_msg = {"message": "User added successfully", "status": True}
                ...
                return Response({"message": "User has no account", "status": False}, status=status.HTTP_400_BAD_REQUEST)
        except UserImages.DoesNotExist as e:
            if request.version == 'v1':
                raise NotFound(detail="UserImage doesn't exist")
            else:
                return Response({"message": "UserImage doesn't exist", "status": False})

    def delete(self, request, id):
        try:
            user = request.user
            # Check If user profile exists
            if ProfileDetails.objects.filter(uuid=user) and ProfileDetails.objects.get(uuid=user).active == True:
                images = UserImages.objects.get(id=id)
                images.liked_user.remove(user)

                image_owner = images.user

                images.save()

                notification = Notification.objects.filter(notification_user=image_owner, activity_type='user',
                                                           activity_sub_type='userimage_liked',
                                                           activity_user=user, user_image_id=images).delete()

                success_msg = {"message": "User removed successfully", "status": True}
                if request.version == 'v1':
...
class GetUserImageLiked(generics.ListAPIView):
    ...
    def get(self, request, id):
        try:
            ...
            # Get all liked user list
            liked_users = image.liked_user.all()
            ...
            # Get all profile details of liked user
            profile_list = ProfileDetails.objects.filter(uuid__in=liked_users)

            queryset = self.filter_queryset(profile_list)
            page = pagination.paginate_queryset(queryset, request)

            profile_details_serializer = ProfileDetailsSerializer(queryset, many=True)
...
class SingleUserImagesComment(views.APIView):

    def get(self, request, id):
        # Get User
        # current_user = User.objects.get(id=3)
        current_user = request.user
        if ProfileDetails.objects.filter(uuid=current_user).exists() and ProfileDetails.objects.get(
            ...
            try:
                ...
                comments = UserImagesComment.objects.get(comment_id=id)

                # Serializer Data
                comment_serializer = UserImagesCommentSerializer(comments)
            ...
            except UserImagesComment.DoesNotExist as e:
                raise NotFound(detail="User Image Comments doesn't exist")
        else:
            return Response({"message: User has no account"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        if ProfileDetails.objects.filter(uuid=current_user).exists() and ProfileDetails.objects.get(
                uuid=current_user).active == True:
            # Create UserImageComments Instance
            comment = UserImagesComment(comment_user=current_user)
            # Pass Comment Instance to Serializer
            comment_serializer = UserImagesCommentSerializer(comment, data=request.data)

            # Check validation of Comment Instance
        ...
        if ProfileDetails.objects.filter(uuid=current_user).exists() and ProfileDetails.objects.get(
            ...
            try:
                ...
                if current_user == comments.comment_user:
                    # Serializer data
                    comment_serializer = UserImagesCommentSerializer(comments, data=request.data, partial=True)
                ...
                raise NotFound(detail="UserImage Comments doesn't exist")
        else:
            return Response({"message: User has no account"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, id):
        ...
        if ProfileDetails.objects.filter(uuid=current_user).exists() and ProfileDetails.objects.get(
            ...
            try:
                ...
                # Verify comment user and delete user
                if current_user == comments.comment_user:
                    # Delete comment
                    comments.delete()
...
class AllUserImageComments(generics.ListAPIView):
    pagination_class = LimitOffsetPagination
    renderer_classes = [JSONRenderer]
    ...
    def get(self, request, image_id):
        try:
            ...
            if ProfileDetails.objects.filter(uuid=current_user).exists() and ProfileDetails.objects.get(
                    uuid=current_user).active == True:
                comment_list = UserImagesComment.objects.filter(user_image=image_id).order_by('created_at')
                page = self.paginate_queryset(comment_list)
                comment_serializer = AllUserImageCommentsSerializer(comment_list, many=True)
...
class UserImageCommentLiked(views.APIView):
    ...
    def post(self, request, comment_id):
        try:
            ...
            if ProfileDetails.objects.filter(uuid=user).exists() and ProfileDetails.objects.get(
                    uuid=user).active == True:
                comment = UserImagesComment.objects.get(comment_id=comment_id)
            ...
            if request.version == 'v1':
                raise NotFound(detail="UserImageComment doesn't exist")
            else:
                return Response({"message": "UserImageComment doesn't exist", "status": False})
...
class AllUserImageCommentLiked(generics.ListAPIView):
    pagination_class = LimitOffsetPagination

    def get(self, request, comment_id):
        try:
            comments = UserImagesComment.objects.get(comment_id=comment_id)
            # Get all liked user list
            liked_user = comments.liked_user.all()

            # Get all profile details of liked user
            profile_list = ProfileDetails.objects.filter(uuid__in=liked_user)
            page = self.paginate_queryset(profile_list)
            profile_details_serializer = ProfileDetailsSerializer(profile_list, many=True)
            return self.get_paginated_response(profile_details_serializer.data)
        except UserImagesComment.DoesNotExist as e:
            raise NotFound(detail="UserImageComments doesn't exist")
        except ProfileDetails.DoesNotExist as e:
            raise NotFound(detail="User Profile Not Found")


class UserProfileImageDetails(generics.ListAPIView):
    pagination_class = LimitOffsetPagination
    ...
    def get(self, request):
        ...
        user = request.user
        user_image = UserImages.objects.filter(user=user).order_by('-create_at')
        page = self.paginate_queryset(user_image)
        user_img_serializer = UserProfileImageDetailsSerializer(user_image, many=True)

        # Updated Sorted Data
        sorted_data = user_img_serializer.data
...
class OtherUserProfileImageDetails(generics.ListAPIView):
    pagination_class = LimitOffsetPagination
