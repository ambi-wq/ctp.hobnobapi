...
from matching.serializers import UserContactsSerializer, UserScoreSerializers, UserFollowerSerializer, \
    UserExploreSerializer, CloseFriendSerializer, InviteFriendsSerializer, SuggesstedUserSerializer
from matching.serializers import LocationSerializer, UserSerializer, InterestedUserProfile, FollowerSerializer, \
    UserSearchSerializer, CloseFriendSearchSerializer
...
class AddFollowerView(views.APIView):
    ...
    def post(self, request):
        ...
        if is_exist == 0:
            ...
            # notification_data = Notification.objects.filter(activity_type="following", activity_user=follower_user)
            notification_data = Notification.objects.filter(activity_sub_type="following", activity_user=follower_user)
            if len(notification_data) > 0:
                notification_data.update(is_read=True)

            suggested_user = User.objects.get(pk=follower_user)
            suggested_user_data = SuggesstedUser.objects.filter(user=following_user, suggessted_user=suggested_user)
            if suggested_user_data:
                suggested_user_data.delete()
    ...
    def delete(self, request, id):
        ...
        try:
            ...
            notification = Notification.objects.filter(
                notification_user=id, activity_type='user', activity_sub_type='following',
...
class FollowerList(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # New Query - filter data based on search
...
class OtherFollowerList(views.APIView):
    ...
    def get(self, request, id):
        try:
            ...
            return paginator.get_paginated_response(serializer.data[start_limit:end_limit])
...
class FollowingList(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # Previous Query
        # queryset = self.get_queryset()
        # New Query - to filter data based on search
        queryset = self.filter_queryset(self.get_queryset())
...
class UserSearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            # Check User Interest
            filter_user = User.objects.all().order_by('-score__desirable_score',
                                                      'geo_location__last_location').prefetch_related('user_profile',
                                                                                                      'user_profile_image',
                                                                                                      'display_pic',
                                                                                                      'user_profile__interest',
                                                                                                      'preference',
                                                                                                      'user_follower',
                                                                                                      'score',
            ...
            print(keywords)
            if keywords:
                filter_user1 = filter_user.filter(Q(user_profile__interest__interest_name__icontains=keywords) | Q(
                    user_profile__full_name__icontains=keywords) | Q(user_profile__username__icontains=keywords))
                pd = ProfileDetails.objects.extra(
                    where=["LOWER(REPLACE(full_name,' ','')) = LOWER(REPLACE(%s,' ',''))"],
                    params=[keywords]).values_list('uuid')
                if pd:
                    filter_user2 = filter_user.filter(user_profile__uuid=pd[0])
                    filter_user = filter_user1 | filter_user2
                else:
                    filter_user = filter_user1
...
class MultipleCloseFriendSearchView(generics.ListAPIView):
    model = User
    serializer_class = CloseFriendSearchSerializer
    pagination_class = LimitOffsetPagination
    ...
    def get_queryset(self):
        ...
        try:
            ...
            close_firends_list = CloseFriendList.objects.values_list(
                'close_friend', flat=True).filter(user=current_user)
            profile_details = ProfileDetails.objects.values_list(
                'uuid', flat=True).filter(Q(username__icontains=keywords) | Q(full_name__icontains=keywords))

            print(f"{profile_details=}")
            # Get following List and remove close friend user from following
            following = UserContacts.objects.values_list(
                'follower', flat=True).filter(following=current_user, follower_id__in=profile_details).exclude(
                follower__in=close_firends_list)

            # Get Follower List and remove close friend and following user if any to prevent duplications
            follower = UserContacts.objects.values_list(
                'following', flat=True).filter(follower=current_user, following_id__in=profile_details).exclude(
                Q(following__in=following) | Q(following__in=close_firends_list))

            profile_details = ProfileDetails.objects.values_list(
                'uuid', flat=True).filter(Q(username__icontains=keywords) | Q(full_name__icontains=keywords)).exclude(
                Q(uuid__in=close_firends_list) | Q(uuid__in=follower) | Q(uuid__in=following))
            print(f"{profile_details=}")

            # Get Profile of Return Follower User
            profile_user = User.objects.filter(id__in=profile_details)
            filter_user = User.objects.filter(Q(id__in=following) | Q(id__in=follower))
            filter_user = filter_user.union(profile_user)
            print("filter_user", filter_user)
        except:
            return []
...
class UserExploreView(generics.ListAPIView):
    ...
    def get_queryset2(self):
        ...
        # Get Current User
        current_user = User.objects.get(username=user)
        print(f"{user=} | {current_user=} | {current_user.id=}")
        try:
            contactsLst = [contact.contact_number for contact in UsersContacts.objects.filter(user=current_user.id)]
            print(f"{contactsLst=}")
            # previous query
    ...
    def list(self, request, *args, **kwargs):
        ...
        contacts_sorted_data = contact_serializer.data
        ...
        # Create New Instance for SuggestedUser upto 15 records
        final_queryset = queryset2 | queryset
        for data in final_queryset[:15]:
            suggessted_user = SuggesstedUser.objects.create(user=user, suggessted_user=data)
            suggested_serailizer = SuggesstedUserSerializer(suggessted_user)

        page = self.paginate_queryset(total_explore_data)
    ...
    def get(self, request):
        ...
        try:
            ...
            if request.version == 'v1':
                ...
                return Response({
                    "message": "User Location Object does not exist. Please Use Post method to create location object."},
...
class GetUserContactsList(generics.ListAPIView):
    model = UsersContacts
    serializer_class = InviteFriendsSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    filterset_fields = ['contact_name', 'contact_number']
    ...
    def get_queryset(self):
        ...
        """
        user = self.request.user
        usercontactlist = UsersContacts.objects.values_list('contact_number').filter(user=user.id)
        exeContactLst = []
        for contact in usercontactlist:
            if ProfileDetails.objects.filter(phone=contact[0]):
                exeContactLst.append(contact[0])
        #
        # return UsersContacts.objects.values('id', 'contact_name', 'contact_number').filter(user=user.id).exclude(
        #     contact_number__in=exeContactLst)

        return UsersContacts.objects.filter(user=user.id).exclude(
            contact_number__in=exeContactLst)
...
def unique(exeContactLst):
    x = np.array(exeContactLst)
    print(np.unique(x))


@api_view(['GET'])
def get_appusers_pnLst(request):
    user = request.user
    usercontactlist = UsersContacts.objects.values_list('contact_number').filter(user=user.id)
    exeContactLst = []
    for contact in usercontactlist:
        if ProfileDetails.objects.filter(phone=contact[0]):
            exeContactLst.append(str(contact[0]))
    print("exeContactLst", exeContactLst)
    return Response(status=status.HTTP_200_OK, data={"message": "Success", "users_list": exeContactLst})


@api_view(['DELETE'])
def delete_suggessted_user(request, id):
    # Get requested user
    user = request.user
    try:
        # Get user from ID
        suggessted_user = User.objects.get(pk=id)
        # Check if user exists
        suggessted_data = SuggesstedUser.objects.get(user=user, suggessted_user=suggessted_user)
        # Delete the record
        suggessted_data.delete()

        # Response with success msg
        success_msg = {"message": "User removed successfully"}
    ...
    except SuggesstedUser.DoesNotExist as e:
        if request.version == 'v1':
            raise NotFound(detail="Requested Suggested User Doesn't Exist")
        else:
            return Response({"message": "Requested Suggested User Doesn't Exist"}, status=status.HTTP_404_NOT_FOUND)
    except User.DoesNotExist as e:
        if request.version == 'v1':
            raise NotFound(detail="Requested User Doesn't Exist")
        else:
...
class DeleteFollower(views.APIView):
    ...
    def delete(self, request, id):
        ...
        try:
            ...
            # Try if connection exist or not
            follower_connection = UserContacts.objects.get(
                following=remove_user, follower=user)
...
class MultipleCloseFriendsListView(generics.ListCreateAPIView, generics.DestroyAPIView):
    model = User
    serializer_class = InterestedUserProfile
    pagination_class = LimitOffsetPagination
    ...
    def get_queryset(self):
        ...
        # Get following List and remove close friend user from following
        following = UserContacts.objects.values_list(
            'follower', flat=True).filter(following=current_user).exclude(follower__in=close_firends_list)

        # Get Follower List and remove close friend and following user if any to prevent duplications
        follower = UserContacts.objects.values_list(
            'following', flat=True).filter(follower=current_user).exclude(
            Q(following__in=following) | Q(following__in=close_firends_list))

        # suggested_list = User.objects.filter(Q(id__in=following) | Q(id__in=follower)).prefetch_related(
        #     'user_profile', 'display_pic', 'user_follower')

        # Get Profiles List with filter Close Friends
        profile_list = User.objects.filter(
            Q(id__in=close_firends_list) | Q(id__in=following) | Q(id__in=follower)).prefetch_related(
            'user_profile', 'user_profile_image', 'user_follower')

        # Return Profile List
        # profile = {'close_friend': profile_list, 'suggested': suggested_list}
        return profile_list
    ...
    def list(self, request, *args, **kwargs):
        ...
        close_friend = CloseFriendList.objects.filter(user=user).values_list('close_friend', flat=True)

        serializer = self.serializer_class(
            queryset, many=True, context={'queryset': queryset_dict, 'close_friend': close_friend})
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            close_friend_user_id = request.data['close_friend']
            close_friend_user_id = json.loads(close_friend_user_id)

            if close_friend_user_id:
                # check already in close friend list
                is_close_friend = CloseFriendList.objects.filter(user=current_user,
                                                                 close_friend_id__in=close_friend_user_id).exists()
                if is_close_friend:
                    message = {"detail": "Already in close friend list", "status": False}
                    if request.version == 'v1':
                    ...
                    # Create new instance
                    for close_friend in close_friend_user_id:
                        CloseFriendList.objects.create(user=current_user, close_friend_id=close_friend).save()

                    message = {"message": "Added CloseFriend successfully", "status": True}
                    if request.version == 'v1':
                ...
                return Response({"message": "Please add close friend id", 'status': False})
        except:
            traceback.print_exc()
            return Response({"message": "Please send close friend id", 'status': False})
        ...
        try:
            ...
            if close_friend_user_id:
                ...
                is_close_friend = CloseFriendList.objects.filter(user=current_user,
                                                                 close_friend_id__in=close_friend_user_id).exists()
                if is_close_friend:
                    for id in close_friend_user_id:
                        # Try get user by id
                        close_friend = CloseFriendList.objects.get(
                            user=current_user, close_friend=id)
                        # User Delete
                        close_friend.delete()
                else:
                    # Send Does not exist response
                    if request.version == 'v1':
                        raise NotFound(detail="User not found in close friend list")
                    else:
                        return Response({"message": "User not found in close friend list"},
                                        status=status.HTTP_404_NOT_FOUND)

                # Response Positivily
                success_msg = {
                    "message": "User removed from close friend list successfully"}
                if request.version == 'v1':
                    response = success_response(success_msg['message'])
                    return Response(response, status=status.HTTP_200_OK)
                else:
...
# Two differnt sections close-friend and  suggressted same as IG
# class MultipleCloseFriendsListView(generics.ListCreateAPIView, generics.DestroyAPIView):
#     model = User
#     serializer_class = InterestedUserProfile
#     pagination_class = LimitOffsetPagination
...
#         queryset = data['close_friend']
#         suggested_queryset = data['suggested']
#
#         queryset_dict = {}
#         for qs_data in queryset:
#             queryset_dict[qs_data.id] = qs_data
#
#         suggested_queryset_dict = {}
#         for qs_data in suggested_queryset:
#             suggested_queryset_dict[qs_data.id] = qs_data
#
#         page = self.paginate_queryset(queryset)
#         page1 = self.paginate_queryset(suggested_queryset)
#
#         serializer = self.serializer_class(
#             queryset, many=True, context={'queryset': queryset_dict})
#
#         # Updated Sorted Data
#         # sorted_data = serializer.data
#         # close_friend = self.get_paginated_response(sorted_data[start_limit:end_limit])
#         close_friend = success_response2(serializer.data)
#
#         suggested_serializer = self.serializer_class(
#             suggested_queryset, many=True, context={'queryset': suggested_queryset_dict})
#
#         # Updated Sorted Data
#         # sorted_data = suggested_serializer.data
#         # suggested = self.get_paginated_response(sorted_data[start_limit:end_limit])
#         suggested = success_response2(suggested_serializer.data)
#
#         return Response(status=status.HTTP_200_OK, data={'close_friend': close_friend, 'suggested': suggested})
